...
class CategoricalDQN:
    ...
    def __init__(self, z_net, n_atoms, v_min, v_max, df=0.99, buffer_len=1e6, batch_size=32,
                 ...
                 start_train_at=4000, results_dir=None, seed=42):
        self.z_net = z_net
    ...
    def train(self, env: gym.Env, n_steps):
        ...
        steps = []
        episode_rewards = []
        state = np_to_unsq_tensor(env.reset(seed=self.seed)[0])
        ...
        for step in loop_range:
            ...
            if done:
                ...
                self.model_reward_hist.append(sum(episode_rewards))
                steps.append(step)
                episode_rewards = []
                loop_range.set_description(f'Reward {rewards[-1]}')

                np.save('rewards.npy', self.model_reward_hist)
                np.save('losses.npy', self.model_loss_hist)
                torch.save(self.z_net.state_dict(), "policy_net_weights.pth")
                torch.save(self._target_net.state_dict(), "target_net_weights.pth")
    ...
    def _train_net(self, states, actions, targets, update):
        ...
        print("Debug #1:", z.shape, targets.shape)
        
        # Compute cross-entropy loss
        loss = -(targets * z.log()).sum(dim=-1).mean()
        self.model_loss_hist.append(loss.item())
        loss.backward()
        self.optimizer.step()
        if update:
    ...
    def _compute_targets(self, rewards, next_states, mask):
        ...
        print("DEBUG #3: ", rewards.shape, atoms.shape, (~mask).sum())
