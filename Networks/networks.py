...
class DistributionalNetwork(LinearNetwork):
    ...
    def __init__(self, inputs, n_actions, n_atoms, n_hidden_layers, n_hidden_units,
                 activation=torch.relu, activation_last_layer=None, dtype=torch.float):
...
class DistributionalNetworkLaplace(LinearNetwork):
    """
    NOTE: We assume that the rewards of the environment are recasted to be non-negative, 
    leading to non-negative Q-values. This allows us to easily enforce that the Q-values
    are non-increasing over increasing sensitivity values
    """

    def __init__(self, inputs, n_actions, n_sensitivities, n_hidden_layers, n_hidden_units,
                 activation=torch.relu, activation_last_layer=torch.sigmoid, dtype=torch.float):
        super(DistributionalNetworkLaplace, self).__init__(inputs=inputs, outputs=n_actions*n_sensitivities,
                                                    n_hidden_layers=n_hidden_layers,
                                                    n_hidden_units=n_hidden_units,
                                                    activation=activation,
                                                    activation_last_layer=activation_last_layer,
    ...
    def forward(self, *inputs):
        x = super(DistributionalNetworkLaplace, self).forward(*inputs)
        #print("DEBUG #9: ", x)
        x = x.reshape(x.shape[0], self.n_actions, self.n_sensitivities)
        #print("DEBUG #6: ", x)
